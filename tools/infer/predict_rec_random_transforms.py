...
def main(args):
    ...
    for image_file in image_file_list:
        ...
        ###Apply transforms
        # transform=tu.get_render_transform()
    ...
    for ino in range(len(img_list)):
        logger.info("Predicts of {}:{}".format(image_file_list[ino],
                                               rec_res[ino]))
    if args.benchmark:
        text_recognizer.autolog.report()
    
    ##SAve results in df
    result_list=[res[0] for res in rec_res]
    results_df = pd.DataFrame({'result':result_list})
    ##Add image name
    results_df['image_path'] = image_file_list
    ##Add ground truth
    ...
    ##add file name
    results_df['file_name'] = results_df['image_path'].apply(lambda x: x.split('/')[-1])
